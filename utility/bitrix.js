...
exports.getConvertedLead = async (req, res) => {
    try {
        ...
        if (!convertedLead) {
            ...
            // If the lead is already converted, append the new invoice entries to the existing invoice array
            if (data.invoice && data.invoice.length > 0) {
                convertedLead.invoice = [...convertedLead.invoice, ...data.invoice];
            }

            // Update other lead data if necessary
            await ConvertedLeadModel.findByIdAndUpdate(convertedLead._id, { $set: { invoice: convertedLead.invoice } }, { new: true });
        }
