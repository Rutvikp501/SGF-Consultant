...
exports.addLead = async (LeadData) => {
     ...
     try {
          ...
          const requestBody = {
               ...
               "fields[UF_CRM_1725958005830]":
                    LeadData.consultant_mobile_no || "",
               "fields[UF_CRM_1725957892961]":
                    LeadData.consultant_email_id || "",
...
exports.getJunkLeadsLead = async () => {
     try {
          ...
          if (lead) {
               let junkLead = await JunkLeadModel.findOne({
                    leadID: lead.leadID,
               });
               if (junkLead) {
               ...
               //await LeadModel.findByIdAndDelete(LeadsId);
               await LeadModel.findByIdAndUpdate(LeadsId, {
                    status: "rejected",
               });
