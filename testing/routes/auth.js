...
router.post("/auth/signup", middleware.ensureNotLoggedIn, async (req,res) => {
	...
	const { User_name, email_id, password1, password2, role } = req.body;
	console.log(req.body);
	
	let errors = [];
	
	if (!User_name  || !email_id || !password1 || !password2) {
	...
	try
	{
		...
		if(user)
		{
			...
			return res.render("auth/signup", {
				title: "User Signup",
				User_name,  errors, email_id, password1, password2
			});
		}
		
		const newUser = new User({ name:User_name,  email_id:email_id, password:password1, role:role });
